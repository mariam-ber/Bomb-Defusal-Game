...
#             return "TBD"
    def __init__(self,component, target, name="Toggles"):
        super().__init__(name, component, target)
        self._prev_value = self.get_value()
    def get_value(self):
...
#                     value+=4
#                 if(self._component[2]):
#                     value+=2
#                 if(self._component[3]):
#                     value+=1
    ...
    def run(self):
        ...
        self._running = True
        while(self._running):
            if(self.get_value() == self._target):
                self._defused = True
            elif(self.get_value() != self._prev_value):
                self._prev_value = self.get_value()
                if(self.get_value() > self._target):
                    self._failed = True
        ...
        while (self._running):
            ...
            # it is released
                ...
                if (self._pressed):
                    ...
                    # for G or B, a specific digit must be in the timer (sec) when releas
                    if(self._color == "G" and self._target in self._timer._min):
                        self._defused = True
                    elif(self._target in self._timer._sec):
                        self._defused = True
                    else:
                        self._failed = True
